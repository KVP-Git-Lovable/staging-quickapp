...
async function getAccessToken(): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(`[${res.status}] Zoho token refresh failed: ${body}`);
  }
  let json: Record<string, unknown>;
  try {
    json = JSON.parse(body);
  } catch {
    throw new Error(`[500] Zoho token response was not JSON: ${body}`);
  }
  if (!json.access_token) {
    // Zoho returns 200 with { error: "invalid_code" } style bodies
    throw new Error(`[400] Zoho token refresh returned no access_token: ${body}`);
  }
