-- Extend finalize_order_edit with payment-intent reconciliation.
-- Backward compatible: both new params default to NULL.
DROP FUNCTION IF EXISTS public.finalize_order_edit(uuid, uuid, uuid, text);
...
DECLARE
  ...
  -- NEW for payment-intent reconciliation
  v_repl_paid numeric;
  v_excess numeric;
  v_delta numeric;
...
BEGIN
  ...
  -- ============================================================
  -- NEW: payment-intent reconciliation for edited order.
  -- Runs after the original is cancelled and the invoice is
  -- superseded, and BEFORE the final recompute_retailer_pending().
  -- ============================================================
  IF p_target_paid IS NOT NULL THEN
    ...
    IF p_target_paid > v_repl_paid THEN
      -- INCREASE: apply newly-collected delta to THIS replacement only.
    ...
    ELSIF p_target_paid < v_repl_paid THEN
      -- DECREASE: peel back excess, reflow to other active orders oldest-first,
      -- leftover becomes retailer ADVANCE credit (NO credit note).
