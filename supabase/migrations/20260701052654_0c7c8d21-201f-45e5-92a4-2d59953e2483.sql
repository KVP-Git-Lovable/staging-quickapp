create or replace function public.activity_visit_action(
  p_visit_id uuid, p_activity_event_id uuid, p_action text,
  p_actor uuid, p_lat double precision default null, p_lng double precision default null)
returns jsonb language plpgsql security definer set search_path to 'public' as $f$
declare v_ci timestamptz; v_dur integer;
begin
  if p_action = 'check_in' then
    update visits set check_in_time = coalesce(check_in_time, now()), status = 'in-progress'
      where id = p_visit_id;
    ...
    update visits set check_out_time = now(), status = 'productive' where id = p_visit_id;
