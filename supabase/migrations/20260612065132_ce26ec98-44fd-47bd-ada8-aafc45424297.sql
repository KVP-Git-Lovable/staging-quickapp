...
BEGIN
  ...
  IF r.verified = true THEN score := score + 10; END IF;
  ...
  -- Once approved (manual approve OR WhatsApp self-confirm) lock the score at 100%.
  IF r.verified = true OR r.whatsapp_verified = true THEN
    score := 100;
  END IF;
...
BEGIN
  FOR rid IN
    ...
    WHERE verified = true OR whatsapp_verified = true
  LOOP
