-- 1) Helper: retailer_in_user_oob_scope
CREATE OR REPLACE FUNCTION public.retailer_in_user_oob_scope(p_retailer_id uuid)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  ...
  v_terr_ok  boolean := false;
BEGIN
  IF v_uid IS NULL OR p_retailer_id IS NULL THEN
    RETURN false;
  END IF;

  SELECT oob_enabled, oob_visibility
    INTO v_cfg
  ...
  IF NOT public.user_has_permission(v_uid, 'order_out_of_beat', 'can_create') THEN
    RETURN false;
  END IF;

  SELECT id, user_id, beat_id, territory_id
    INTO v_r
  ...
  IF v_cfg.oob_visibility = 'beat' THEN
    RETURN false;
  END IF;

  IF v_cfg.oob_visibility IN ('assigned','territory','all') THEN
    v_assigned :=
      (v_r.user_id = v_uid)
      OR public.user_has_beat_access(v_uid, v_r.beat_id)
      OR EXISTS (
        SELECT 1 FROM public.retailer_shared_access rsa
         WHERE rsa.retailer_id = p_retailer_id
           AND rsa.shared_to_user_id = v_uid
           AND rsa.is_active = true
           AND COALESCE(rsa.can_take_orders, false) = true
      );
    IF v_assigned THEN
      RETURN true;
    END IF;
  ...
  IF v_cfg.oob_visibility IN ('territory','all') AND v_r.territory_id IS NOT NULL THEN
    v_terr_ok := EXISTS (
      SELECT 1
        FROM public.beats b
       WHERE b.territory_id = v_r.territory_id
         AND (
           b.user_id = v_uid
           OR public.user_has_beat_access(v_uid, b.beat_id)
         )
    );
    IF v_terr_ok THEN
      RETURN true;
  ...
  IF v_cfg.oob_visibility = 'all' THEN
    IF public.user_has_permission(v_uid, 'order_out_of_beat', 'can_view_all') THEN
      RETURN true;
    END IF;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.retailer_in_user_oob_scope(uuid) TO authenticated;

-- 2) Extend retailers_select policy (keep all existing branches + OR oob scope)
...
USING (
  ...
  AND (
    ...
    OR public.retailer_in_user_oob_scope(id)
  )
);

-- 3) Update sync_order_with_items_v2: OOB guard, credit rule, notify, snapshots
...
BEGIN
  ...
  IF v_oob AND COALESCE(v_cfg.oob_notify_manager, false) THEN
