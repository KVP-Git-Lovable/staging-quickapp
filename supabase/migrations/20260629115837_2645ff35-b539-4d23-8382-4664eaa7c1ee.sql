...
-- =========================================================
-- A) Harden sync_order_with_items_v2: resolve product_id / variant_id robustly
-- =========================================================
...
BEGIN
  ...
  -- Per-item resolution: build v_resolved_items with hard product_id + variant_id
  FOR v_item IN SELECT jsonb_array_elements(v_items) LOOP
    ...
    -- Skip free items with explicit null product_id and "(FREE)" suffix gracefully:
    -- they may not have any matching product. We still try resolution but won't
    -- reject if it's clearly a free-item placeholder (product_name endswith "(FREE)").
    DECLARE
      v_is_free_placeholder boolean := (v_pname IS NOT NULL AND v_pname ILIKE '%(FREE)');
    BEGIN
      -- 1) Try direct product_id
      ...
      -- 3) Fallback by product_name (case-insensitive, trimmed)
      ...
      -- 4) Reject if still unresolved (unless free-item placeholder)
      IF v_resolved_pid IS NULL THEN
        IF v_is_free_placeholder THEN
          -- allow null product_id for promotional free lines
          v_resolved_pid := NULL;
        ELSE
      ...
      ELSIF v_pid_raw IS NOT NULL AND v_resolved_pid IS NULL THEN
        -- Legacy: caller put a variant id into product_id field. Try resolving as variant.
        BEGIN
          SELECT id INTO v_resolved_vid FROM public.product_variants WHERE id = v_pid_raw::uuid;
          IF v_resolved_vid IS NOT NULL THEN
            -- Pop the last error (we'll allow this since the variant was found)
            -- Resolve its parent product as the product_id.
            SELECT product_id INTO v_resolved_pid FROM public.product_variants WHERE id = v_resolved_vid;
            -- Strip the most recent "product not found" error
            v_errors := (
...
-- =========================================================
-- C) Data repair: re-link the 3 recent orphans whose product still exists
-- =========================================================
UPDATE public.order_items
   SET product_id = 'e217736e-95d2-4d17-bd3b-c5770f93c2ca'
 WHERE id = '1b738bd7-c20b-40b3-8931-c45130cc0c9f'
   AND product_id IS NULL
   AND product_name = 'Coffee Toffee 500ml';

UPDATE public.order_items
   SET product_id = 'd45a0322-9839-4cf4-adb5-eff77f7043e5'
 WHERE id = 'd9544d88-92f6-4f52-b301-3dd7bb2944a5'
   AND product_id IS NULL
   AND product_name = 'Ball 100ml';

UPDATE public.order_items
   SET product_id = '2c4e8af7-b35a-4e0c-b16a-68dc42227543'
 WHERE id = '3e9f46f0-aff0-4fd5-9bb1-bc93803e9873'
   AND product_id IS NULL
   AND product_name = 'Strawberry In Strawberry';
