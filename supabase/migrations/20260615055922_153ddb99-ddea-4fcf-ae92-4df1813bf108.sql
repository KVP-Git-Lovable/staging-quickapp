...
BEGIN
  ...
  FOR v_item IN SELECT jsonb_array_elements(v_items) LOOP
    ...
    -- Require at least one of product_id / variant_id to identify the SKU
    IF v_pid IS NULL AND v_vid IS NULL THEN
      v_errors := v_errors || to_jsonb('item.product_id or variant_id required'::text); CONTINUE;
    END IF;

    -- Validate product_id if provided (allow either products.id or variant id)
  ...
  -- FIX: order_items has no product_id column; insert only variant_id.
  -- If the payload's product_id is actually a variant id, use it as variant_id.
  INSERT INTO public.order_items (
    order_id, variant_id, product_name, category, rate, unit, quantity, total,
