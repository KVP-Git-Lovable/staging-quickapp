-- Add {status} token support in notif_fill
...
RETURNS text LANGUAGE sql STABLE AS $$
  SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
    ...
    '{date}',       COALESCE(p_meta->>'date', to_char(now(),'YYYY-MM-DD'))),
    '{time}',       COALESCE(p_meta->>'time','')),
    '{points}',     COALESCE(p_meta->>'points','0')),
    '{beat}',       COALESCE(p_meta->>'beat','')),
    '{status}',     COALESCE(p_meta->>'status',''));
$$;

-- Emit status in the attendance trigger metadata
...
BEGIN
  BEGIN
    PERFORM public.emit_notification_event(
      ...
      jsonb_build_object(
        'date',   to_char(COALESCE(NEW.date, current_date),'YYYY-MM-DD'),
        'time',   to_char(COALESCE(NEW.check_in_time, now()),'HH12:MI am'),
