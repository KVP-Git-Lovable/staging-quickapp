...
-- 1. can_backdate_order
CREATE OR REPLACE FUNCTION public.can_backdate_order(p_date date)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  IF NOT public.user_has_permission(auth.uid(), 'order_backdate', 'can_create') THEN
    RETURN false;
  END IF;
  ...
  IF p_date >= (CURRENT_DATE - (COALESCE(v_max_days, 0) || ' days')::interval)::date THEN
    RETURN true;
  END IF;

  RETURN EXISTS (
    SELECT 1
      FROM public.order_backdate_date_grants
...
GRANT EXECUTE ON FUNCTION public.can_backdate_order(date) TO authenticated;

-- 2 & 3. sync_order_with_items_v2 — additive: backdated columns + server-side guard
...
BEGIN
  ...
  -- Server-side guard: backdated orders require can_backdate_order for the effective order_date.
