-- Dispatch agent assignment foundation
ALTER TABLE public.packing_lists
  ADD COLUMN IF NOT EXISTS assigned_agent_id uuid,
  ADD COLUMN IF NOT EXISTS assigned_delivery_user_id uuid REFERENCES public.distributor_users(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS driver_phone text;

CREATE INDEX IF NOT EXISTS idx_packing_lists_assigned_agent_id ON public.packing_lists(assigned_agent_id);

-- Update primary dispatch RPC to persist driver/agent identity from jsonb
...
BEGIN
  ...
  -- Validate delivery user (must belong to this distributor, active, can_deliver)
