  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Carry the original order's backdated date forward into the edit session —
        // otherwise getEffectiveOrderDate() falls back to today and the replacement
        // order silently loses the backdated order_date.
    ...
    try {
      ...
      const freeOrderItems = orderCalculation.appliedSchemes
        ...
        .flatMap(s => s.free_items!.map(freeItem => ({
          ...
          variant_id: (typeof freeItem.variant_id === 'string' && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(freeItem.variant_id)) ? freeItem.variant_id : null,
