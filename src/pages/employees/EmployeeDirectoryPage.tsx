                ...
                {filtered.map(r => (
                  <div key={r.id} className="rounded-lg border p-3 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      <button onClick={() => openDetail(r)} className="text-left flex-1 min-w-0">
                        <p className="font-medium text-indigo-600 truncate">{r.full_name}</p>
                        <p className="text-xs text-muted-foreground truncate">
                          {[r.employee_code, r.department].filter(Boolean).join(' · ') || '—'}
                        </p>
                      </button>
                      {r.follows_company_page ? <Badge>Follows</Badge> : <Badge variant="outline">—</Badge>}
                    </div>
                    <div className="text-xs text-muted-foreground truncate">{r.email || '—'} · {r.phone || '—'}</div>
                    <div className="flex items-center justify-between gap-2 pt-1 border-t">
                      <span className="text-xs">Market Portal</span>
                      <div className="flex items-center gap-2">
                        ...
                        />
                        {r.portal_enabled && (
                          <a href={`/employee-portal/login?preview=${r.id}`} target="_blank" rel="noreferrer"
                  ...
                  {filtered.map(r => (
                    <TableRow key={r.id}>
                      <TableCell className="font-medium">
                        <button onClick={() => openDetail(r)} className="text-indigo-600 hover:underline text-left">
                          {r.full_name}
                        </button>
                      </TableCell>
